#pragma once
#include "Predefine.h"


#define SINGLETON_DECLARE( Type )						\
private:												\
	static Type*		m_psSelf;						\
public:													\
	static HADES_FORCEINLINE Type* Create()					\
	{													\
		ASSERT( NULL == m_psSelf );						\
		m_psSelf = new Type;							\
		return m_psSelf;								\
	}													\
	static HADES_FORCEINLINE void Destroy()					\
	{													\
		delete m_psSelf;								\
	}													\
	static HADES_FORCEINLINE Type* Instance()					\
	{													\
		return m_psSelf;								\
	}	

#define SINGLETON_DECLARE_ONCREATE_ONDESTROY( Type )	\
private:												\
	static Type*		m_psSelf;						\
public:													\
	static HADES_FORCEINLINE Type* Create()					\
	{													\
		ASSERT( NULL == m_psSelf );						\
		m_psSelf = new Type;							\
		m_psSelf->_OnCreate();							\
		return m_psSelf;								\
	}													\
	static HADES_FORCEINLINE void Destroy()					\
	{													\
		m_psSelf->_OnDestroy();							\
		delete m_psSelf;								\
		m_psSelf = NULL;								\
	}													\
	static HADES_FORCEINLINE Type* Instance()					\
	{													\
		return m_psSelf;								\
	}													\


#define SINGLETON_IMPLEMENT( Type )	Type* Type::m_psSelf = NULL;




#define SYSTEM_SINGLETON_DECLEAR( type )\
	private:\
	static type*			s_rpSelf;\
	public:\
	static HADES_FORCEINLINE type* Instance()\
		{\
		return s_rpSelf;\
		}

#define SYSTEM_SINGLETON_IMPLEMENT( type ) type* type::s_rpSelf = NULL;
#define SYSTEM_SINGLETON_INITIALIZE ASSERT(NULL==s_rpSelf);s_rpSelf = this;
#define SYSTEM_SINGLETON_DESTROY s_rpSelf = NULL;