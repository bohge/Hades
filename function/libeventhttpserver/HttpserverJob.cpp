...
namespace hlhsp
{
	...
	//---------------------------------------------------------------------------------------------------------
	void HttpserverJob::Pushhandle(IAgent* agent)
	{
		if (NULL != m_pHttp)
		{
	...
	//---------------------------------------------------------------------------------------------------------
	bool HttpserverJob::Initialize(HttpserverJob::ServerType st, uint port, bool isauto)
	{
		string crt;
		string key;
		...
		if (ST_HTTPS == st)
		{
			...
			* See http://www.mail-archive.com/openssl-dev@openssl.org/msg30957.html */
			...
			server_setup_certs(ctx, crt.c_str(), key.c_str());

			/* This is the magic that lets evhttp use SSL. */
			evhttp_set_bevcb(m_pHttp, bevcb, ctx);
		}
		...
		* cb.  We can also add callbacks for specific paths. */
		...
		m_pHandle = evhttp_bind_socket_with_handle(m_pHttp, "0.0.0.0", port);
		if (!m_pHandle) {
			HADESERROR("couldn't bind to port %d. Exiting.", (int)port);
		}
