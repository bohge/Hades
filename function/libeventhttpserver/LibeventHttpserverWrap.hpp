...
namespace hlhsp
{
	...
	//-------------------------------------------------------------------------------------------------------
	static int LibeventHttpserverFunction_StartServer(hc::IVirtualMachine* vm, hc::FunctionRegister* func, Httpserver* self, uint* type, uint* port, bool* isauto)
	{
		self->StartServer((HttpserverJob::ServerType)*type, *port, *isauto);
		return 0;
	}
	//-------------------------------------------------------------------------------------------------------
	...
	static void Register_LibeventHttpserverFunction(const eastl::string& name, hc::IScriptSystem& sys)
	{
...

		{
			...
			sf.Arguments(&hc::UInt::RTTI(), &hc::UInt::RTTI(), &hc::Bool::RTTI());
			sf.Function(&hlhsp::LibeventHttpserverFunction_StartServer, "StartServer");
			sc.ClassFunction(sf);
		}
