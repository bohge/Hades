...
static void bcwrite_instructions(BCWriteCtx *ctx, GCproto *pt)
{
	MSize sizedbg = 0;
	struct DumpBuffer* buffer = (struct DumpBuffer*)ctx->wdata;
	/* Recursively write children of prototype. */
	if ((pt->flags & PROTO_CHILD)) {
		ptrdiff_t i, n = pt->sizekgc;
		GCRef *kr = mref(pt->k, GCRef) - 1;
		for (i = 0; i < n; i++, kr--) {
			GCobj *o = gcref(*kr);
			if (o->gch.gct == ~LJ_TPROTO)
				bcwrite_instructions(ctx, gco2pt(o));
		}
	}
	/* Write bytecode instructions and upvalue refs. */
	bcwrite_bytecode(ctx, &buffer->buffer[buffer->size], pt);
	buffer->size += (pt->sizebc - 1)*(MSize)sizeof(BCIns);
}

/* Protected callback for bytecode writer. */
static TValue *cpinstructionswriter(lua_State *L, lua_CFunction dummy, void *ud)
{
	BCWriteCtx *ctx = (BCWriteCtx *)ud;
	UNUSED(L); UNUSED(dummy);
	lj_buf_need(&ctx->sb, 1024);  /* Avoids resize for most prototypes. */
	bcwrite_instructions(ctx, ctx->pt);
	return NULL;
}

int lj_bcinstructions(lua_State *L, GCproto *pt, void *data)
{
	BCWriteCtx ctx;
	int status;
	ctx.pt = pt;
	...
	ctx.status = 0;
	lj_buf_init(L, &ctx.sb);
	status = lj_vm_cpcall(L, NULL, &ctx, cpinstructionswriter);
	if (status == 0) status = ctx.status;
	lj_buf_free(G(sbufL(&ctx.sb)), &ctx.sb);
	return status;
}

int LuaL_dempinstructions(lua_State *L, void *data)
{
	cTValue *o = L->top - 1;
	api_check(L, L->top > L->base);
	if (tvisfunc(o) && isluafunc(funcV(o)))
		return lj_bcinstructions(L, funcproto(funcV(o)), data);
	else
		return 1;
}

#include <stdio.h>
LUA_API int LuaL_havereturn(lua_State* L)
{
	int res = 0;
	struct DumpBuffer dbuffer;
	dbuffer.size = 0;
	if (lua_isfunction(L, -1))
	{
		if (0 != LuaL_dempinstructions(L, &dbuffer))
		{
			printf("error lua_dump");
		}
		else
		{
			BCIns* op = (BCIns*)dbuffer.buffer;
			uint32_t count = dbuffer.size >> 2;
