...
#include <pthread.h>
static pthread_key_t s_pKey = NULL;

void SetLuastate(lua_State *L)
{
	if (NULL == s_pKey)
	{
		int ret = pthread_key_create(&s_pKey, NULL);
		if (ret != 0)
		{
			printf(" can not init TLS Slot ");
		}
	}
	pthread_setspecific(s_pKey, L);
}
...
{
	//lua_State *L = data;
	lua_State *L = pthread_getspecific(s_pKey);
