...
namespace hfbs
{
	...
	//---------------------------------------------------------------------------------------------------------
	hc::IEncoder* FlatEncoder::GetChild()
	{
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(const char* name, uint nlen, const ISerializable* v)
	{
		ASSERT(NULL != v);
		uint factor = v->GetFactor();
		auto flatname = m_rpBuilder->CreateString(name, nlen);
		auto flatkey = CreateTString(*m_rpBuilder, flatname);
		auto flatbuff = m_rpBuilder->CreateVector(v->GetBuffer(), v->GetLength());
		auto flatvalue = CreateTNative(*m_rpBuilder, factor, flatbuff);
		auto flatmember = CreateMember(*m_rpBuilder, Type_TString, flatkey.Union(), Type_TNative, flatvalue.Union());
		m_Members.push_back(flatmember);
	}
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(const char* name, uint nlen, const hc::IEncoder* v)
	{
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(const char* name, uint nlen, ushort v)
	{
		auto flatname = m_rpBuilder->CreateString(name, nlen);
		auto flatkey = CreateTString(*m_rpBuilder, flatname);
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(int index, const ISerializable* v)
	{
		ASSERT(NULL != v);
		uint factor = v->GetFactor();
		auto flatkey = CreateTInt(*m_rpBuilder, index);
		auto flatbuff = m_rpBuilder->CreateVector(v->GetBuffer(), v->GetLength());
		auto flatvalue = CreateTNative(*m_rpBuilder, factor, flatbuff);
		auto flatmember = CreateMember(*m_rpBuilder, Type_TInt, flatkey.Union(), Type_TNative, flatvalue.Union());
		m_Members.push_back(flatmember);
	}
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(int index, const hc::IEncoder* v)
	{
		const FlatEncoder* flatencoder = static_cast<const FlatEncoder*>(v);
		auto flatkey = CreateTInt(*m_rpBuilder, index);
		auto members = flatencoder->GetMemberVector();
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(int index, ushort v)
	{
		auto flatkey = CreateTInt(*m_rpBuilder, index);
		auto flatvalue = CreateTUShort(*m_rpBuilder, v);
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(uint64 index, const ISerializable* v)
	{
		ASSERT(NULL != v);
		uint factor = v->GetFactor();
		auto flatkey = CreateTUInt64(*m_rpBuilder, index);
		auto flatbuff = m_rpBuilder->CreateVector(v->GetBuffer(), v->GetLength());
		auto flatvalue = CreateTNative(*m_rpBuilder, factor, flatbuff);
		auto flatmember = CreateMember(*m_rpBuilder, Type_TUInt64, flatkey.Union(), Type_TNative, flatvalue.Union());
		m_Members.push_back(flatmember);
	}
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(uint64 index, const hc::IEncoder* v)
	{
		const FlatEncoder* flatencoder = static_cast<const FlatEncoder*>(v);
		auto flatkey = CreateTUInt64(*m_rpBuilder, index);
		auto members = flatencoder->GetMemberVector();
	...
	//---------------------------------------------------------------------------------------------------------
	void FlatEncoder::SetField(uint64 index, ushort v)
	{
		auto flatkey = CreateTUInt64(*m_rpBuilder, index);
		auto flatvalue = CreateTUShort(*m_rpBuilder, v);
	...
	//---------------------------------------------------------------------------------------------------------
	flatbuffers::Offset< flatbuffers::Vector< flatbuffers::Offset< hfbs::Member > > > FlatEncoder::GetMemberVector() const
	{
