...
local training_frequency = 4; --每经过多少step进行一次训练
local training_batch_size = 32; --一次随机梯度下降计算的个数
local synchronize_step = 2000; --没过多少step同步
local switch_frequency = 1000; --目标网络与训练网络切换频率 
local learn_start = 1000; --开始训练的经验个数
local max_experience_pool = 2^20 --经验池大小
...
function qlearcentrnode:Setup()
  if _ENABLE_TENSOR_BOARD then
    ...
    self.cc:remove_experiment("pddqn_cnn");
  ...
  self.neuronmaker = IFileSystem:ReadFile("comm:script/arclight/neuronnets/pac_dueling_cnn_lite_1_15_15.lua");
...
function qlearcentrnode:Trainning(lr, its)
  ...
  for _, node in pairs(self.clusternodes) do
    ...
    node:Trainning(its, training_frequency, training_batch_size, switch_frequency, learn_start, max_experience_pool, experiences_discount, synchronize_step);
  end  
  self.steploss = 0;
...
function qlearcentrnode:EpsiodeDone(episode, episodereward, ps, pb, str)
