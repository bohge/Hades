...
local training_times = 100;
local training_package_size = 8  --每一个数据集的第一维大小
local training_batch_size = 10   --一次随机梯度下降计算的个数
local training_frequency = training_package_size * training_batch_size --每经过多少帧训练一次
local max_experience_pool = training_package_size * training_batch_size * 300 --经验池大小
...
function qlearcentrnode:InsertComputernode(strgid, node)
  ...
  node:SetParameters(experiences_discount, training_package_size, action_count(), get_pac_state_size());
...
--插入经验
function qlearcentrnode:_InsertExperience(current_state, next_state, action, reward)
...
function qlearcentrnode:Trainning(lr, its)
  ...
  for episode = 1, its do
    ...
    for stepcount=1, max_experience_pool do
      ...
      local reward = keypressed_playing(action);
      episodereward = episodereward + reward;
      local next_state = get_pac_state(self.meanstd);
      self:_InsertExperience(self.current_state, next_state, action, reward);
...
function qlearcentrnode:_PortionTraining(stepcount)
  if self:_isNeedTrainning(stepcount) then
    for i=1,training_times do
      ...
      LOG(string.format("partion %d times trainning loss %.4f", i, loss));
    end    
...
function qlearcentrnode:_doTrainning(partdata)
  ...
  assert( sendcount % training_package_size == 0);--必须能被整出  
  ...
  for _, node in pairs(self.clusternodes) do
    ...
    node:SynchronyNeuronnet(ps, pb, ss, sb);
...
function qlearcentrnode:CollectResult(loss, gs, gb, ss, sb)
...
function qlearcentrnode:_SerializeSubparameters()
