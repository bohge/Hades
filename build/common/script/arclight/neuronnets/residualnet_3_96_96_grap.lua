...
function MakeNeuronCriterion()
  ...
  input = nn.Identity()()
  ------> 3, 96,96
  model = nn.SpatialConvolution(3, 16, 5,5, 2,2, 2,2)
              :init('weight', nninit.kaiming, {gain = 'relu'})
              :init('bias', nninit.constant, 0)(input)
  ...
  for i=1,N-1 do   model = _getResidualLayer(model, 128)   end
  ------> 128, 1,1     Pooling, Linear, Softmax
  model = nn.SpatialAveragePooling(6,6)(model)
  model = nn.Reshape(128)(model)
  model = nn.Linear(128, 1)(model)
