...
function aaaccentrnode:Setup()
  ...
  self.neuronmaker = IFileSystem:ReadFile("comm:script/arclight/neuronnets/a3c_gym_continuous.lua");
  ...
  self.ActorNeuron, self.CriticNeuron = MakeNeuronCriterion(ss.shape[1], as.shape[1], 2);  
  
  self.savefile = IFileSystem:PathAssembly("docs:save/a3c_pendulum_%s.tensor");
...
function aaaccentrnode:_ApplyGradient(neuronnet, config, sb, bb)
  ...
  --config.learningRate = self.learning_rate;
