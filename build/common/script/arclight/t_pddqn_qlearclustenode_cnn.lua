...
function qlearclustenode:_CaculateExperiences(transition)
  ...
  self.neuronnet:evaluate();
  for _,v in ipairs(transition) do
    local current_out = self.neuronnet:forward(v.current_state):clone();
    local updatereward = current_out:clone();
    if v.isterminal then
      updatereward[{v.action}] = v.reward;
    else
      local next_out = self.target_q:forward(v.next_state):clone();
      local next_eval = self.neuronnet:forward(v.next_state);--double dqn
      local _, next_action = torch.max(next_eval, 1);
      updatereward[{v.action}] = v.reward + experiences_discount * next_out[next_action[1]];
    end
    batch = { input = v.current_state, target = updatereward }
    local priority = math.abs(current_out[{v.action}] - updatereward[{v.action}]);
