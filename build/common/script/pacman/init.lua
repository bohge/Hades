...
--ai reward
reward_ghost_score = 50;
reward_dots_score = 10
reward_superdots_score = 20
reward_alive = 1;
reward_hit_wall = -10;
reward_dead = -100;
reward_scaler = 50;
...
function caculate_mean_std(info)
  ...
  if collectindex == collectcount then
    ...
    local meanstdpath = currentpath.."/preprocess.table";
...
function setup_tensor()
  local w = #(map[1]);
  local h = #map;
  mixdata = torch.Tensor(collectcount, 1, w, h);
  collectindex = 1;
end
...
function get_pac_state(meanstd)
  ...
  for k, character in pairs(characters) do
    if not character:is_dead() then
      local x = math.floor(character.y);
      local oy = math.floor(character.x);
      local y = math.min(19, math.max(1, oy));
      --if oy < 1 or oy > h then print(info); end 
      --print(string.format("error character postion %s\t\t%d\t%d", character.color, x, y));
      if x >= 1 and x <= w
        and y >= 1 and y <= h then
        if character.shape == 'hero' then
          info[x][y] = defiend_hero;
        else
          if character:is_weak() then
            if character:is_inverted_weak() then
              info[x][y] = defiend_trunghost;
            else
              info[x][y] = defiend_weakghost;
            end            
          else
            info[x][y] = defiend_ghost;
          end
        end
      else
        ERROR(string.format("error character postion %s %.3f %.3f", character.shape, character.x, character.y));
      end  
    end  
  end  
  info[{{},{}}]:add(-meanstd[1]);
  info[{{},{}}]:div(meanstd[2]);
  --return info:view(1,1,w,h);
  return info:view(1,w,h);
  --return info;
end