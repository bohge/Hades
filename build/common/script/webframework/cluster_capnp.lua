-- Generated by lua-capnproto 0.1.3-1 on Thu Mar 23 23:03:37 2017
...
_M.IncomeCluster = {
    id = "16690723030890941419",
    displayName = "cluster.capnp:IncomeCluster",
    dataWordCount = 1,
    pointerCount = 1,
    discriminantCount = 0,
    ...
    calc_size = function(data)
        ...
        return size + _M.IncomeCluster.calc_size_struct(data)
    end,
    ...
    flat_serialize = function(data, p32, pos)
        ...
        value = data["ipadress"]
        if type(value) == "string" then
            local data_off = get_data_off(_M.IncomeCluster, 0, pos)

            local len = #value + 1
            write_listp_buf(p32, _M.IncomeCluster, 0, 2, len, data_off)
    ...
    serialize = function(data, p8, size)
        if not p8 then
            size = _M.IncomeCluster.calc_size(data)
        ...
        -- skip header
        write_structp(p32 + 2, _M.IncomeCluster, 0)

        -- skip header & struct pointer
        _M.IncomeCluster.flat_serialize(data, p32 + 4)
    ...
    parse_struct_data = function(p32, data_word_count, pointer_count, header,
        ...
        -- text
        local off, size, num = read_listp_struct(p32, header, _M.IncomeCluster, 0)
        if off and num then
    ...
    parse = function(bin, tab)
        ...
        if off and dw and pw then
            return _M.IncomeCluster.parse_struct_data(p32 + 2 + off * 2, dw, pw,
                    header, tab)
        else
            return nil
        end
