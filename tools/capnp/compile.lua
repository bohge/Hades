...
function comp_parse_struct_data(res, nodes, struct, fields, size, name)
    ...
    parse_struct_data = function(p32, memsize, data_word_count, pointer_count, header,
            tab)
    ...
    if struct.discriminantCount and struct.discriminantCount > 0 then
        ...
        local dscrm = _M.%s.which(p32, memsize, %d)]], name, struct.discriminantOffset))
    end
    ...
    for i, field in ipairs(fields) do
        ...
        _M.%s["%s"].parse_struct_data(p32, memsize, _M.%s.dataWordCount,
        ...
        -- enum
        local val = read_struct_field(p32, memsize, "uint16", %d, %d)
        ...
        -- list
        local off, size, num = read_listp_struct(p32, memsize, header, _M.%s, %d)
        if off and num then
			
            -- dataWordCount + offset + pointerSize + off
            s["%s"] = read_list_data(p32 + (%d + %d + 1 + off) * 2, memsize - (%d + %d + 1 + off) * 2 * 4, header,
                    num, %s)
        else
            s["%s"] = nil
        end
]], name, off, field.name, struct.dataWordCount, off, struct.dataWordCount, off, types, field.name))
		...
		local m = memsize - (%d + %d) * 2 * 4
        local off, dw, pw = read_struct_buf(p, m, header)
        if off and dw and pw then
            ...
            _M.%s.parse_struct_data(p + 2 + off * 2, m - 8 - off * 8, dw, pw, header, s["%s"])
        else
            s["%s"] = nil
        end
]], struct.dataWordCount, off, struct.dataWordCount, off, field.name, field.name, field.type_display_name,
...
]], name, off, struct.dataWordCount, off, struct.dataWordCount, off, field.name, field.name))
        ...
        if off and num then
			...
			if  memsize - (%d + %d + 1 + off) * 2 * 4 < 0 then			
				error("out of range");		
			end
        ...
        s["%s"] = read_struct_field(p32, memsize, "%s", %d, %d, %s)
...
function comp_parse(res, name)
    ...
    parse = function(bin, memsize, tab)
        if memsize < 16 then
        ...
        local off, dw, pw = read_struct_buf(p32, memsize, header)
        if off and dw and pw then
		
			if  memsize < 2 + off * 2 * 4 then			
				error("out of range");		
			end

            return _M.%s.parse_struct_data(p32 + 2 + off * 2, memsize - (2 - off * 2) * 4, dw, pw,
                    header, tab)
        else
            return nil
        end
...
function comp_flat_serialize(res, nodes, struct, fields, size, name)
    ...
    if struct.discriminantCount and struct.discriminantCount ~= 0 then
        ...
        if dscrm then
            ...
            _M.%s.which(p32, memsize, %d, dscrm)
        end
...
function comp_which(res)
    ...
    which = function(buf, memsize, offset, n)
        if n then
            -- set value
            write_struct_field(buf, n, "uint16", 16, offset)
        else
            -- get value
            return read_struct_field(buf, memsize, "uint16", 16, offset)
        end
    end,
]])
end
