...
-- @return              parsed field
function _M.read_struct_field(p32, memsize, field_type, size, off, default)
	...
	print("tttt2+++"..field_type)
	print("tttt2+++"..memsize)
	
    local val
    if size >= 8 then
		if  memsize < (size * off + size) / 8 then		
			error("out of range");	
		end
        local n, s = get_bit_offset(size * off, size)
        val = p[n]
    else
		if  memsize < (size * off + 8) / 8 then		
			error("out of range");	
		end
...
function _M.read_text_data(buf, memsize, num)
	if  memsize < num	then		
		error("out of range");	
	end
...
function _M.read_text(buf, memsize, header, T, offset, default)
    local res
    local data_off, size, num = _M.read_listp_struct(buf, memsize, header, T, 2)
    if data_off and num then
	
		if  header.memsize < num - 1 then		
			error("out of range");	
		end
		
		if  memsize < num - 1 then		
			error("out of range");	
		end
...
function _M.read_listp(p32, memsize, header)
    local val0 = p32[0]
    local val1 = p32[1]
    ...
    elseif sig == 2 then
        ...
        return _M.read_far_pointer(p32, memsize, header, _M.read_listp)
    else
...
function _M.read_list_data(p32, memsize, header, num, elm_type, ...)
    ...
    if elm_type == "list" then
        ...
        for i = 1, num do
            local off, child_size, child_num = _M.read_listp_list(p32, memsize, 
                    header, i)
            if off and child_num then
			
				if  header.memsize  < (i + off) * 2 then
					error("out of range");	
				end
				
				if  memsize  < (i + off) * 2 then
					error("out of range");	
				end
				
                t[i] = _M.read_list_data(p32 + (i + off) * 2, memsize - (i + off) * 2, header,
                        child_num, ...)
            end
        end
    ...
    elseif elm_type == "text" then
        ...
        for i = 1, num do
            ...
            if off and child_num then
				...
				if  memsize  < (i + off) * 2 * 4 then
					error("out of range");	
				end
				
                t[i] = _M.read_text_data(p32 + (i + off) * 2, memsize - (i + off) * 2 * 4, child_num - 1)
            end
        end
    elseif elm_type == "data" then
        ...
        for i = 1, num do
            ...
            if off and child_num then
                ...
                t[i] = _M.read_text_data(p32 + (1 + off) * 2, memsize - (i + off) * 2 * 4, child_num)
            end
        end
    elseif elm_type == "struct" then
        ...
        for i = 1, real_num do
            ...
            t[i] = new_tab(0, 8)
				...
				if  memsize < memsize - 8 - (i - 1) * struct_size then
					error("out of range");	
				end
				
            T.parse_struct_data(p32 + 2 + (i - 1) * struct_size / 4, memsize - 8 - (i - 1) * struct_size, dt,
...
-- @index: start from 1
function _M.read_listp_list(p32, memsize, header, index)
	if  memsize  < (index - 1) * 2 * 4 then
		error("out of range");	
	end
    return _M.read_listp(p32 + (index - 1) * 2, memsize - (index - 1) * 2 * 4, header)
end

function _M.read_listp_struct(buf, memsize, header, T, offset)
    local p = cast(pint32, buf)
    local base = T.dataWordCount * 2 + offset * 2
	if  memsize  < base then
		error("out of range");	
	end
    return _M.read_listp(p + base, memsize - base, header)
end

function _M.read_far_pointer(buf, memsize, header, parser)
    local p = buf
...
function _M.read_struct_buf(p32, memsize, header)
    local p = p32
    ...
    elseif sig == 2 then
        return _M.read_far_pointer(p, memsize, header, _M.read_struct_pointer)
    else
