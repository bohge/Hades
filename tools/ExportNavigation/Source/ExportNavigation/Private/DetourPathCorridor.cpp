...
namespace original
{
	...
	int dtMergeCorridorStartMoved(odtPolyRef* path, const int npath, const int maxPath,
		const odtPolyRef* visited, const int nvisited)
	{
		int furthestPath = -1;
		...
		if (req + size > maxPath)
			size = maxPath - req;
		if (size)
			memmove(path + req, path + orig, size * sizeof(odtPolyRef));
	...
	int dtMergeCorridorEndMoved(odtPolyRef* path, const int npath, const int maxPath,
		const odtPolyRef* visited, const int nvisited)
	{
		int furthestPath = -1;
		int furthestVisited = -1;
		...
		if (count)
			memcpy(path + ppos, visited + vpos, sizeof(odtPolyRef)*count);

		return ppos + count;
	}

	int dtMergeCorridorStartShortcut(odtPolyRef* path, const int npath, const int maxPath,
		const odtPolyRef* visited, const int nvisited)
	{
		int furthestPath = -1;
		int furthestVisited = -1;
	...
	*/
	...
	{
		dtAssert(!m_path);
		m_path = (odtPolyRef*)dtAlloc(sizeof(odtPolyRef)*maxPath, DT_ALLOC_PERM);
		if (!m_path)
			return false;
		m_npath = 0;
		m_maxPath = maxPath;
	...
	void dtPathCorridor::reset(odtPolyRef ref, const float* pos)
	{
		dtAssert(m_path);
		dtVcopy(m_pos, pos);
	...
	*/
	int dtPathCorridor::findCorners(float* cornerVerts, unsigned char* cornerFlags,
		odtPolyRef* cornerPolys, const int maxCorners,
		dtNavMeshQuery* navquery, const dtQueryFilter* /*filter*/)
	{
		...
		while (ncorners)
		{
			...
			if (ncorners)
			{
				...
				memmove(cornerPolys, cornerPolys + 1, sizeof(odtPolyRef)*ncorners);
	...
	*/
	...
	bool dtPathCorridor::moveOverOffmeshConnection(odtPolyRef offMeshConRef, odtPolyRef* refs,
		...
		dtNavMeshQuery* navquery)
	{
		...
		odtPolyRef prevRef = 0, polyRef = m_path[0];
		int npos = 0;
	...
	*/
	bool dtPathCorridor::movePosition(const float* npos, dtNavMeshQuery* navquery, const dtQueryFilter* filter)
	{
		...
		odtPolyRef visited[MAX_VISITED];
		int nvisited = 0;
	...
	*/
	...
	void dtPathCorridor::setCorridor(const float* target, const odtPolyRef* path, const int npath)
	{
		dtAssert(m_path);
		dtAssert(npath > 0);
		...
		memcpy(m_path, path, sizeof(odtPolyRef)*npath);
		m_npath = npath;
	}

	bool dtPathCorridor::fixPathStart(odtPolyRef safeRef, const float* safePos)
	{
		dtAssert(m_path);
	...
	bool dtPathCorridor::trimInvalidPath(odtPolyRef safeRef, const float* safePos,
		dtNavMeshQuery* navquery, const dtQueryFilter* filter)
	{
		dtAssert(navquery);
		dtAssert(filter);
