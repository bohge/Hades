...
#ifndef ORIGINAL_DETOURCROWD_H
#define ORIGINAL_DETOURCROWD_H
...
namespace original
{
	...
	struct dtCrowdAgent
	{
		...
		odtPolyRef cornerPolys[DT_CROWDAGENT_MAX_CORNERS];

		/// The number of corners.
		int ncorners;

		unsigned char targetState;			///< State of the movement request.
		odtPolyRef targetRef;				///< Target polyref of the movement request.
	...
	class dtCrowd
	{
		...
		bool requestMoveTargetReplan(const int idx, odtPolyRef ref, const float* pos);
	...
	public:
		...
		bool requestMoveTarget(const int idx, odtPolyRef ref, const float* pos);
