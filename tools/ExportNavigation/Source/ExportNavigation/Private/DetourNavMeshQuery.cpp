...
bool dtQueryFilter::passFilter(const odtPolyRef /*ref*/,
...
float dtQueryFilter::getCost(const float* pa, const float* pb,
							 const odtPolyRef /*prevRef*/, const dtMeshTile* /*prevTile*/, const dtPoly* /*prevPoly*/,
							 const odtPolyRef /*curRef*/, const dtMeshTile* /*curTile*/, const dtPoly* curPoly,
							 const odtPolyRef /*nextRef*/, const dtMeshTile* /*nextTile*/, const dtPoly* /*nextPoly*/) const
{
	return dtVdist(pa, pb) * m_areaCost[curPoly->getArea()];
}
#else
inline bool dtQueryFilter::passFilter(const odtPolyRef /*ref*/,
									  const dtMeshTile* /*tile*/,
									  const dtPoly* poly) const
{
	return (poly->flags & m_includeFlags) != 0 && (poly->flags & m_excludeFlags) == 0;
...
dtStatus dtNavMeshQuery::findRandomPoint(const dtQueryFilter* filter, float (*frand)(),
										 odtPolyRef* randomRef, float* randomPt) const
{
	dtAssert(m_nav);
	...
	const dtPoly* poly = 0;
	odtPolyRef polyRef = 0;
	const odtPolyRef base = m_nav->getPolyRefBase(tile);
	...
	for (int i = 0; i < tile->header->polyCount; ++i)
	{
		...
		if (p->getType() != DT_POLYTYPE_GROUND)
			continue;
		// Must pass filter
		const odtPolyRef ref = base | (odtPolyRef)i;
...
dtStatus dtNavMeshQuery::findRandomPointAroundCircle(odtPolyRef startRef, const float* centerPos, const float maxRadius,
													 ...
													 odtPolyRef* randomRef, float* randomPt) const
{
	...
	while (!m_openList->empty())
	{
		...
		const odtPolyRef bestRef = bestNode->id;
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			odtPolyRef neighbourRef = link->ref;
...
///
dtStatus dtNavMeshQuery::closestPointOnPoly(odtPolyRef ref, const float* pos, float* closest, bool* posOverPoly) const
{
	dtAssert(m_nav);
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
...
/// 
dtStatus dtNavMeshQuery::closestPointOnPolyBoundary(odtPolyRef ref, const float* pos, float* closest) const
{
	dtAssert(m_nav);
...
/// of the polygon.
/// 
dtStatus dtNavMeshQuery::getPolyHeight(odtPolyRef ref, const float* pos, float* height) const
{
	dtAssert(m_nav);
...
public:
	...
	odtPolyRef nearestRef() const { return m_nearestRef; }
	const float* nearestPoint() const { return m_nearestPoint; }

	void process(const dtMeshTile* tile, dtPoly** polys, odtPolyRef* refs, int count)
	{
		dtIgnoreUnused(polys);

		for (int i = 0; i < count; ++i)
		{
			odtPolyRef ref = refs[i];
...
dtStatus dtNavMeshQuery::findNearestPoly(const float* center, const float* extents,
										 ...
										 odtPolyRef* nearestRef, float* nearestPt) const
{
	dtAssert(m_nav);
										 ...
										 const dtQueryFilter* filter, dtPolyQuery* query) const
{
	...
	if (tile->bvTree)
	{
		...
		while (node < end)
		{
			...
			{
				odtPolyRef ref = base | (odtPolyRef)node->i;
...
public:
	dtCollectPolysQuery(odtPolyRef* polys, const int maxPolys)
	...
	void process(const dtMeshTile* tile, dtPoly** polys, odtPolyRef* refs, int count)
	{
		...
		memcpy(m_polys + m_numCollected, refs, (size_t)toCopy * sizeof(odtPolyRef));
...
dtStatus dtNavMeshQuery::queryPolygons(const float* center, const float* extents,
									   const dtQueryFilter* filter,
									   odtPolyRef* polys, int* polyCount, const int maxPolys) const
{
...
///
dtStatus dtNavMeshQuery::findPath(odtPolyRef startRef, odtPolyRef endRef,
								  const float* startPos, const float* endPos,
								  const dtQueryFilter* filter,
								  odtPolyRef* path, int* pathCount, const int maxPath) const
{
	dtAssert(m_nav);
	dtAssert(m_nodePool);
	dtAssert(m_openList);
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			odtPolyRef neighbourRef = bestTile->links[i].ref;
...
dtStatus dtNavMeshQuery::getPathToNode(dtNode* endNode, odtPolyRef* path, int* pathCount, int maxPath) const
{
...
/// path query.
///
dtStatus dtNavMeshQuery::initSlicedFindPath(odtPolyRef startRef, odtPolyRef endRef,
											const float* startPos, const float* endPos,
...
dtStatus dtNavMeshQuery::updateSlicedFindPath(const int maxIter, int* doneIters)
{
	...
	while (iter < maxIter && !m_openList->empty())
	{
		...
		odtPolyRef parentRef = 0, grandpaRef = 0;
		const dtMeshTile* parentTile = 0;
		const dtPoly* parentPoly = 0;
		dtNode* parentNode = 0;
		if (bestNode->pidx)
...
dtStatus dtNavMeshQuery::finalizeSlicedFindPath(odtPolyRef* path, int* pathCount, const int maxPath)
{
	*pathCount = 0;
...
dtStatus dtNavMeshQuery::finalizeSlicedFindPathPartial(const odtPolyRef* existing, const int existingSize,
													   odtPolyRef* path, int* pathCount, const int maxPath)
{
	*pathCount = 0;
...
dtStatus dtNavMeshQuery::appendVertex(const float* pos, const unsigned char flags, const odtPolyRef ref,
									  float* straightPath, unsigned char* straightPathFlags, odtPolyRef* straightPathRefs,
...
dtStatus dtNavMeshQuery::appendPortals(const int startIdx, const int endIdx, const float* endPos, const odtPolyRef* path,
									  float* straightPath, unsigned char* straightPathFlags, odtPolyRef* straightPathRefs,
									  int* straightPathCount, const int maxStraightPath, const int options) const
{
	...
	{
		// Calculate portal
		const odtPolyRef from = path[i];
		const dtMeshTile* fromTile = 0;
		const dtPoly* fromPoly = 0;
		if (dtStatusFailed(m_nav->getTileAndPolyByRef(from, &fromTile, &fromPoly)))
			return DT_FAILURE | DT_INVALID_PARAM;
		
		const odtPolyRef to = path[i+1];
...
dtStatus dtNavMeshQuery::findStraightPath(const float* startPos, const float* endPos,
										  const odtPolyRef* path, const int pathSize,
										  float* straightPath, unsigned char* straightPathFlags, odtPolyRef* straightPathRefs,
										  int* straightPathCount, const int maxStraightPath, const int options) const
{
	dtAssert(m_nav);
	...
	*straightPathCount = 0;
	...
	if (pathSize > 1)
	{
		...
		odtPolyRef leftPolyRef = path[0];
		odtPolyRef rightPolyRef = path[0];
...
/// position.
///
dtStatus dtNavMeshQuery::moveAlongSurface(odtPolyRef startRef, const float* startPos, const float* endPos,
										  const dtQueryFilter* filter,
										  float* resultPos, odtPolyRef* visited, int* visitedCount, const int maxVisitedSize) const
{
	dtAssert(m_nav);
	...
	*visitedCount = 0;
	...
	while (nstack)
	{
		...
		// Get poly and tile.
		// The API input has been cheked already, skip checking internal data.
		const odtPolyRef curRef = curNode->id;
		...
		for (int i = 0, j = (int)curPoly->vertCount-1; i < (int)curPoly->vertCount; j = i++)
		{
			...
			else if (curPoly->neis[j])
			{
				...
				const odtPolyRef ref = m_nav->getPolyRefBase(curTile) | idx;
...
dtStatus dtNavMeshQuery::getPortalPoints(odtPolyRef from, odtPolyRef to, float* left, float* right,
...
dtStatus dtNavMeshQuery::getPortalPoints(odtPolyRef from, const dtPoly* fromPoly, const dtMeshTile* fromTile,
										 odtPolyRef to, const dtPoly* toPoly, const dtMeshTile* toTile,
...
dtStatus dtNavMeshQuery::getEdgeMidPoint(odtPolyRef from, odtPolyRef to, float* mid) const
{
	float left[3], right[3];
...
dtStatus dtNavMeshQuery::getEdgeMidPoint(odtPolyRef from, const dtPoly* fromPoly, const dtMeshTile* fromTile,
										 odtPolyRef to, const dtPoly* toPoly, const dtMeshTile* toTile,
										 float* mid) const
{
	float left[3], right[3];
...
///
dtStatus dtNavMeshQuery::raycast(odtPolyRef startRef, const float* startPos, const float* endPos,
								 const dtQueryFilter* filter,
								 float* t, float* hitNormal, odtPolyRef* path, int* pathCount, const int maxPath) const
{
	dtRaycastHit hit;
	hit.path = path;
	hit.maxPath = maxPath;
...
dtStatus dtNavMeshQuery::raycast(odtPolyRef startRef, const float* startPos, const float* endPos,
								 const dtQueryFilter* filter, const unsigned int options,
								 dtRaycastHit* hit, odtPolyRef prevRef) const
{
	dtAssert(m_nav);
...
/// filled to capacity.
/// 
dtStatus dtNavMeshQuery::findPolysAroundCircle(odtPolyRef startRef, const float* centerPos, const float radius,
											   const dtQueryFilter* filter,
											   odtPolyRef* resultRef, odtPolyRef* resultParent, float* resultCost,
...
///
dtStatus dtNavMeshQuery::findPolysAroundShape(odtPolyRef startRef, const float* verts, const int nverts,
											  const dtQueryFilter* filter,
											  odtPolyRef* resultRef, odtPolyRef* resultParent, float* resultCost,
											  int* resultCount, const int maxResult) const
{
...
dtStatus dtNavMeshQuery::getPathFromDijkstraSearch(odtPolyRef endRef, odtPolyRef* path, int* pathCount, int maxPath) const
{
...
/// be filled to capacity.
/// 
dtStatus dtNavMeshQuery::findLocalNeighbourhood(odtPolyRef startRef, const float* centerPos, const float radius,
												const dtQueryFilter* filter,
												odtPolyRef* resultRef, odtPolyRef* resultParent,
												int* resultCount, const int maxResult) const
{
	dtAssert(m_nav);
	dtAssert(m_tinyNodePool);
	...
	*resultCount = 0;
	...
	while (nstack)
	{
		...
		for (unsigned int i = curPoly->firstLink; i != DT_NULL_LINK; i = curTile->links[i].next)
		{
			...
			for (int j = 0; j < n; ++j)
			{
				odtPolyRef pastRef = resultRef[j];
...
static void insertInterval(dtSegInterval* ints, int& nints, const int maxInts,
						   const short tmin, const short tmax, const odtPolyRef ref)
{
...
/// 
dtStatus dtNavMeshQuery::getPolyWallSegments(odtPolyRef ref, const dtQueryFilter* filter,
											 float* segmentVerts, odtPolyRef* segmentRefs, int* segmentCount,
...
///
dtStatus dtNavMeshQuery::findDistanceToWall(odtPolyRef startRef, const float* centerPos, const float maxRadius,
											const dtQueryFilter* filter,
	...
	while (!m_openList->empty())
	{
		...
		for (int i = 0, j = (int)bestPoly->vertCount-1; i < (int)bestPoly->vertCount; j = i++)
		{
			...
			else if (bestPoly->neis[j])
			{
				...
				const odtPolyRef ref = m_nav->getPolyRefBase(bestTile) | idx;
...
bool dtNavMeshQuery::isValidPolyRef(odtPolyRef ref, const dtQueryFilter* filter) const
{
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
...
/// 
bool dtNavMeshQuery::isInClosedList(odtPolyRef ref) const
{
