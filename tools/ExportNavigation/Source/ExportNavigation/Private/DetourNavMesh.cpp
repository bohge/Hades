...
namespace original
{
	...
	*/
		...
		odtPolyRef* con, float* conarea, int maxcon) const
	{
		...
		odtPolyRef base = getPolyRefBase(tile);
	...
	void dtNavMesh::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side)
	{
		...
		for (int i = 0; i < target->header->offMeshConCount; ++i)
		{
			...
			float nearestPt[3];
			odtPolyRef ref = findNearestPolyInTile(tile, p, ext, nearestPt);
			if (!ref)
				continue;
			...
			if (targetCon->flags & DT_OFFMESH_CON_BIDIR)
			{
				...
				if (tidx != DT_NULL_LINK)
				{
					...
					link->ref = getPolyRefBase(target) | (odtPolyRef)(targetCon->poly);
					link->edge = 0xff;
	...
	void dtNavMesh::connectIntLinks(dtMeshTile* tile)
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			for (int j = poly->vertCount - 1; j >= 0; --j)
			{
				...
				{
					dtLink* link = &tile->links[idx];
					link->ref = base | (odtPolyRef)(poly->neis[j] - 1);
					link->edge = (unsigned char)j;
					link->side = 0xff;
					link->bmin = link->bmax = 0;
					// Add to linked list.
	...
	void dtNavMesh::baseOffMeshLinks(dtMeshTile* tile)
	{
		...
		for (int i = 0; i < tile->header->offMeshConCount; ++i)
		{
			...
			if (tidx != DT_NULL_LINK)
			{
				...
				dtPoly* landPoly = &tile->polys[landPolyIdx];
				dtLink* link = &tile->links[tidx];
				link->ref = base | (odtPolyRef)(con->poly);
				link->edge = 0xff;
				link->side = 0xff;
				link->bmin = link->bmax = 0;
				// Add to linked list.
	...
	void dtNavMesh::closestPointOnPoly(odtPolyRef ref, const float* pos, float* closest, bool* posOverPoly) const
	{
	...
	odtPolyRef dtNavMesh::findNearestPolyInTile(const dtMeshTile* tile,
		...
		float* nearestPt) const
	{
		...
		odtPolyRef polys[128];
		int polyCount = queryPolygonsInTile(tile, bmin, bmax, polys, 128);

		// Find nearest polygon amongst the nearby polygons.
		odtPolyRef nearest = 0;
		float nearestDistanceSqr = FLT_MAX;
		for (int i = 0; i < polyCount; ++i)
		{
			odtPolyRef ref = polys[i];
		...
		odtPolyRef* polys, const int maxPolys) const
	{
		if (tile->bvTree)
		{
			...
			while (node < end)
			{
				...
				{
					if (n < maxPolys)
						polys[n++] = base | (odtPolyRef)node->i;
				}
	...
	/// reference it had previously used.  In this case the #odtPolyRef's for the
		...
		dtTileRef lastRef, dtTileRef* result)
	{
		...
		else
		{
			...
			int tileIndex = (int)decodePolyIdTile((odtPolyRef)lastRef);
			...
			// Restore salt.
			tile->salt = decodePolyIdSalt((odtPolyRef)lastRef);
		}
	...
	const dtMeshTile* dtNavMesh::getTileByRef(dtTileRef ref) const
	{
		if (!ref)
			return 0;
		unsigned int tileIndex = decodePolyIdTile((odtPolyRef)ref);
		unsigned int tileSalt = decodePolyIdSalt((odtPolyRef)ref);
	...
	dtStatus dtNavMesh::getTileAndPolyByRef(const odtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
	{
	...
	void dtNavMesh::getTileAndPolyByRefUnsafe(const odtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
	{
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
		*tile = &m_tiles[it];
	...
	bool dtNavMesh::isValidPolyRef(odtPolyRef ref) const
	{
		if (!ref) return false;
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
	...
	/// @code
	///
	/// const odtPolyRef base = navmesh->getPolyRefBase(tile);
	/// for (int i = 0; i < tile->header->polyCount; ++i)
	/// {
	...
	/// }
	/// @endcode
	odtPolyRef dtNavMesh::getPolyRefBase(const dtMeshTile* tile) const
	{
		if (!tile) return 0;
		const unsigned int it = (unsigned int)(tile - m_tiles);
	...
	///
	/// Tile state includes non-structural data such as polygon flags, area ids, etc.
	/// @note This function does not impact the tile's #dtTileRef and #odtPolyRef's.
	/// @see #storeTileState
	...
	dtStatus dtNavMesh::getOffMeshConnectionPolyEndPoints(odtPolyRef prevRef, odtPolyRef polyRef, float* startPos, float* endPos) const
	{
		unsigned int salt, it, ip;
	...
	const dtOffMeshConnection* dtNavMesh::getOffMeshConnectionByRef(odtPolyRef ref) const
	{
		unsigned int salt, it, ip;
	...
	dtStatus dtNavMesh::setPolyFlags(odtPolyRef ref, unsigned short flags)
	{
		if (!ref) return DT_FAILURE;
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
	...
	dtStatus dtNavMesh::getPolyFlags(odtPolyRef ref, unsigned short* resultFlags) const
	{
		if (!ref) return DT_FAILURE;
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
	...
	dtStatus dtNavMesh::setPolyArea(odtPolyRef ref, unsigned char area)
	{
		if (!ref) return DT_FAILURE;
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
	...
	dtStatus dtNavMesh::getPolyArea(odtPolyRef ref, unsigned char* resultArea) const
	{
		if (!ref) return DT_FAILURE;
		unsigned int salt, it, ip;
		decodePolyId(ref, salt, it, ip);
