...
#ifndef ORIGINAL_DETOUTPATHCORRIDOR_H
#define ORIGINAL_DETOUTPATHCORRIDOR_H
...
namespace original
{
	...
	public:
		...
		void reset(odtPolyRef ref, const float* pos);
		...
		bool moveOverOffmeshConnection(odtPolyRef offMeshConRef, odtPolyRef* refs,
			float* startPos, float* endPos,
			dtNavMeshQuery* navquery);

		bool fixPathStart(odtPolyRef safeRef, const float* safePos);

		bool trimInvalidPath(odtPolyRef safeRef, const float* safePos,
			dtNavMeshQuery* navquery, const dtQueryFilter* filter);
		...
		void setCorridor(const float* target, const odtPolyRef* polys, const int npath);
		...
		inline odtPolyRef getFirstPoly() const { return m_npath ? m_path[0] : 0; }

		/// The polygon reference id of the last polygon in the corridor, the polygon containing the target.
		/// @return The polygon reference id of the last polygon in the corridor. (Or zero if there is no path.)
		inline odtPolyRef getLastPoly() const { return m_npath ? m_path[m_npath - 1] : 0; }

		/// The corridor's path.
		/// @return The corridor's path. [(polyRef) * #getPathCount()]
		inline const odtPolyRef* getPath() const { return m_path; }
