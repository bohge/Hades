...
bool dtCrowd::init(const int maxAgents, const float maxAgentRadius, dtNavMesh* nav)
{
	...
	m_maxPathResult = 256;
	m_pathResult = (odtPolyRef*)dtAlloc(sizeof(odtPolyRef)*m_maxPathResult, DT_ALLOC_PERM);
	if (!m_pathResult)
		return false;
...
bool dtCrowd::requestMoveTargetReplan(const int idx, odtPolyRef ref, const float* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
bool dtCrowd::requestMoveTarget(const int idx, odtPolyRef ref, const float* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
...
void dtCrowd::updateMoveRequest(const float /*dt*/)
{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		{
			const odtPolyRef* path = ag->corridor.getPath();
			const int npath = ag->corridor.getPathCount();
			dtAssert(npath);

			static const int MAX_RES = 32;
			float reqPos[3];
			odtPolyRef reqPath[MAX_RES];	// The path to the request location
			int reqPathCount = 0;
			...
			else if (dtStatusSucceed(status))
			{
				...
				if (valid)
				{
					...
					if (npath > 1)
					{
						...
						memmove(res+npath-1, res, sizeof(odtPolyRef)*nres);
						// Copy old path in the beginning.
						memcpy(res, path, sizeof(odtPolyRef)*(npath-1));
						nres += npath-1;
						...
						for (int j = 0; j < nres; ++j)
						{
							if (j-1 >= 0 && j+1 < nres)
							{
								if (res[j-1] == res[j+1])
								{
									memmove(res+(j-1), res+(j+1), sizeof(odtPolyRef)*(nres-(j+1)));
									nres -= 2;
									j -= 2;
								}
							}
...
void dtCrowd::checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		float agentPos[3];
		odtPolyRef agentRef = ag->corridor.getFirstPoly();
