...
#ifndef ORIGINAL_DETOURNODE_H
#define ORIGINAL_DETOURNODE_H
...
namespace original
{
	...
	struct dtNode
	{
		...
		odtPolyRef id;								///< Polygon ref the node corresponds to.
	};
	...
	public:
		...
		dtNode* getNode(odtPolyRef id, unsigned char state = 0);
		dtNode* findNode(odtPolyRef id, unsigned char state);
		unsigned int findNodes(odtPolyRef id, dtNode** nodes, const int maxNodes);
