...
#ifndef ORIGINAL_DETOURPATHQUEUE_H
#define ORIGINAL_DETOURPATHQUEUE_H
...
namespace original
{
	...
	public:
		...
		dtPathQueueRef request(odtPolyRef startRef, odtPolyRef endRef,
			const float* startPos, const float* endPos,
			const dtQueryFilter* filter);

		dtStatus getRequestStatus(dtPathQueueRef ref) const;

		dtStatus getPathResult(dtPathQueueRef ref, odtPolyRef* path, int* pathSize, const int maxPath);
