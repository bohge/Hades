...
namespace original
{
	...
	bool dtPathQueue::init(const int maxPathSize, const int maxSearchNodeCount, dtNavMesh* nav)
	{
		...
		for (int i = 0; i < MAX_QUEUE; ++i)
		{
			...
			m_queue[i].path = (odtPolyRef*)dtAlloc(sizeof(odtPolyRef)*m_maxPathSize, DT_ALLOC_PERM);
			if (!m_queue[i].path)
				return false;
		}
	...
	dtPathQueueRef dtPathQueue::request(odtPolyRef startRef, odtPolyRef endRef,
	...
	dtStatus dtPathQueue::getPathResult(dtPathQueueRef ref, odtPolyRef* path, int* pathSize, const int maxPath)
	{
		for (int i = 0; i < MAX_QUEUE; ++i)
		{
			if (m_queue[i].ref == ref)
			{
				...
				memcpy(path, q.path, sizeof(odtPolyRef)*n);
				*pathSize = n;
